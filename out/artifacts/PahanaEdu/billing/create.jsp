...
<head>
    ...
    <style>
        ...
        .stock-alert { background: #fffbeb; border: 1px solid #f59e0b; color: #92400e; padding: 8px 12px; border-radius: 6px; margin-top: 8px; font-size: 0.9rem; }
...
<script>
    ...
    function addItemRow() {
        ...
        var html = ''+
            ...
            '<input type="number" class="form-control qty" name="quantity" min="1" value="1" oninput="validateStock(this)" required />'+
            '</div>'+
            '<div class="col-md-2">'+
    ...
    function onItemChange(id){
        ...
        if(select && unit && qty){
            ...
            // Set custom validation for stock limit
            if(stock > 0){
                qty.max = stock;
                qty.setAttribute('data-stock', stock);

                // Set custom validation message
                qty.setCustomValidity('');
                qty.title = 'Only ' + stock + ' items available in stock';

                if(parseInt(qty.value) > stock){
                    qty.value = stock;
                    showStockAlert(row, stock);
                } else {
                    hideStockAlert(row);
                }
            } else {
                qty.removeAttribute('data-stock');
                qty.title = '';
                hideStockAlert(row);
            }
            recalcRow(id);
    ...
    function validateStock(input) {
        ...
        if (value > stock) {
            input.setCustomValidity('Only ' + stock + ' items available in stock');
            showStockAlert(row, stock);
        } else {
    ...
    function showStockAlert(row, stock) {
        // Remove any existing alert
        hideStockAlert(row);

        // Create and show alert
        var alertDiv = document.createElement('div');
        alertDiv.className = 'stock-alert mt-2';
        alertDiv.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Only ' + stock + ' items available in stock';
        alertDiv.id = 'stock-alert-' + row.id;

        row.appendChild(alertDiv);
    }

    function hideStockAlert(row) {
        var existingAlert = document.getElementById('stock-alert-' + row.id);
        if (existingAlert) {
    ...
    document.getElementById('billForm').addEventListener('submit', function(e){
        ...
        rows.forEach(function(r){
            var qtyInput = r.querySelector('.qty');
            if (qtyInput && qtyInput.value) {
                var stock = parseInt(qtyInput.getAttribute('data-stock') || 0);
                var value = parseInt(qtyInput.value) || 0;

                if (value > stock) {
                    hasStockError = true;
                    showStockAlert(r, stock);
                }
            }

            if(r.querySelector('.item-select').value){
                hasItem=true;
            }
        });
        ...
        if(hasStockError){
            alert('Some items have quantity exceeding available stock. Please adjust quantities before proceeding.');
            e.preventDefault();
            return;
        }
    });
